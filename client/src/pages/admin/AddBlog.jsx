  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
      <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
        ...
        <label htmlFor="image">
          <img src={!image ? assets.upload_area : URL.createObjectURL(image)} alt="upload-icon" className='mt-2 h-16 rounded cursor-pointer'/>
        ...
        <div className='max-w-lg h-74 pb-16 sm:pb-10 pt-2 relative'>
          ...
          <button type='button' onClick={generateContent} className='absolute bottom-1 right-2 ml-2 text-xs text-white bg-black/70 px-4 py-1.5 rounded hover:bg-primary/80 cursor-pointer'>Generate with AI</button>
        </div>
